 ...
 */
@SpringBootTest @AutoConfigureMockMvc class SearchAiApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("知识数据源"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanPlanPermissionAwareSearch()throws Exception{mvc.perform(post("/api/ai/search/plan").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"query\":\"交付验收流程\",\"userDepartments\":[\"delivery\"],\"requestedSources\":[\"delivery:project\",\"finance:budget\",\"public:policy\"],\"requireCitations\":true}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.permittedSources.length()").value(2)).andExpect(jsonPath("$.data.blockedSources[0]").value("finance:budget"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
